    ...
    public partial class PayrollDetails : Form
    {
        ...
        public DateRangeDto PayrollDateRange
        {
            ...
            set
            {
                ...
                _endDate = DateOnly.ParseExact(value.EndDate, "yyyy-MM-dd");
                PayrollPeriodLabel.Text = $"{_startDate:MMMM d, yyyy} - {_endDate: MMMM d, yyyy}";
        ...
        public PayrollDetails(MainForm mainForm)
        {
            InitializeComponent();
            _mainForm = mainForm;
            ViewCount = 10;
        ...
        private async void LoadView(List<PersonalInformationDisplayDto> data)
        {
            ...
            {
                CalculateMaxPage();
                var activeEmployeeData = data.Where(x => x.IsActive).ToList();
                ActiveEmployeeCount.Text = data.Count(x => x.IsActive).ToString();
                var paginatedData = activeEmployeeData.Skip(SkipCount).Take(ViewCount).ToList();
                PageCountLabel.Text = data.Count == 0 ? "0 of 0" :
                                      _currentPage == _maxPage ? $"{SkipCount + 1} - {activeEmployeeData.Count} of {activeEmployeeData.Count}" :
                                      $"{SkipCount + 1} - {ViewCount + SkipCount} of {activeEmployeeData.Count}";
                CurrentPageLabel.Text = _currentPage.ToString();
        ...
        private async void LoadPayrollData(ApiResponse<List<PayrollDto>> apiData)
        {
            try
            {
                ...
                if (apiData.isSuccess)
                {
                    ...
                    await UpdatePayrollView(PayrollList);
                    TotalExpenses = (Decimal)PayrollList.Sum(x => x.NetSalary) + (Decimal)PayrollList.Sum(x => x.EmployeePagibigShare) + (Decimal)PayrollList.Sum(x => x.EmployeePhilhealthShare) + (Decimal)PayrollList.Sum(x => x.EmployeeSssShare)
                                    + (Decimal)PayrollList.Sum(x => x.EmployerPagibigShare) + (Decimal)PayrollList.Sum(x => x.EmployerPhilhealthShare) + (Decimal)PayrollList.Sum(x => x.EmployerSssShare);
                    TotalExpensesLabel.Text = $"PHP {TotalExpenses:#,#0.00}";
                }
                else
                {
        ...
        private void ViewDetailsButton_Click(object sender, EventArgs e)
        {
            ...
            {
                var payrollDetailModal = new PayrollDetailsModal(SelectedPayrollData, SelectedEmployee, _mainForm);
        ...
        {
            var result = GunaMessage.Question(_mainForm, "Generating payslips will make the payroll info fixed, confirm?", "Confirm");
