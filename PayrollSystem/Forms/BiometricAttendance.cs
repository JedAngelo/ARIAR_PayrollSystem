...
namespace PayrollSystem.Forms
{
    public partial class BiometricAttendance : Form
    {
        ...
        public BiometricAttendance(MainForm mainForm, bool standAlone)
        {
            InitializeComponent();
            LogoutButton.Visible = standAlone;
            _mainForm = mainForm;
        ...
        private async void LogAttendance(Guid personalId)
        {
            try
            {
                ...
                {                    
                    errorMessage = "Invalid, existing record found";
                    if (_attendance.MorningOut == null && _attendance.MorningIn != null && timeNow >= _morningOutStart && timeNow <= _morningOutEnd)
                    {
                        ...
                        if (timeDifference <= new TimeSpan(1, 0, 0))
                        {
                            ...
                            //{
                            //    ControlsHelper.SpeakTextAsync("Invalid, already timed in", _ttsVoice);
                            //    return;
                            //}
                            ControlsHelper.SpeakTextAsync("Invalid, already timed in", _ttsVoice);
                            return;
                ...
                if (attendanceData != null)
                {
                    ...
                    if (result.isSuccess)
                    {
                        ...
                        ControlsHelper.SpeakTextAsync(messageToSpeak, _ttsVoice);
                    }
                    else
                    {
                        ToastNotify.Error(result.ErrorMessage);
                        ControlsHelper.SpeakTextAsync("Attendance Failed!", _ttsVoice);
                    }
                }
                else
                {
                    ControlsHelper.SpeakTextAsync(errorMessage, _ttsVoice);
        ...
        public void OnCaptured(CaptureResult captureResult)
        {
            try
            {
                ...
                if (employeeFmds == null || employeeFmds.Count == 0)
                {
                    ...
                    //ControlsHelper.SpeakTextAsync("No ");
                    var errorMessage = "No employee biometrics found";
                    ControlsHelper.SpeakTextAsync(errorMessage, _ttsVoice);
                    MakeReport(errorMessage, "WARNING");
                    return;
                }
                ...
                else
                {
                    ...
                    ControlsHelper.SpeakTextAsync("Invalid Fingerprint!", _ttsVoice);
        ...
        private async void BiometricAttendance_Load(object sender, EventArgs e)
        {
            if (_mainForm.Settings != null)
            {
                _ttsVoice = _mainForm.Settings.TTSVoice;
            }
            else
            {
        ...
        private void TimeoutStart()
        {
            ...
            {
                // Trigger previous timer's callback and dispose it
                _timeoutTimer.Dispose(); // prevent race conditions, dispose before callback
                TimeoutCallback(null);
                _timeoutTimer = null; // set to null before new timer is started

            }

            _timeoutTimer = new System.Threading.Timer(TimeoutCallback, null, 2000, Timeout.Infinite);
        ...
        private void TimeoutCallback(object state)
        {
            ...
            Form dialogForm = Form.ActiveForm; //get the currently active form
            ...
            if (dialogForm != null)
            {
                ...
                {
                    BeginInvoke((Action)(() => {
                        dialogForm.Activate(); //bring the current dialog into focus using Activate
                        SendKeys.Send("c");
                    }));
            ...
            {
                Console.WriteLine("TimeoutCallback: No Active Form Found!");
            }
