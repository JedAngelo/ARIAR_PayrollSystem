...
namespace PayrollSystem.Forms.Modals
{
    public partial class AttendanceLogModal : Form
    {
        ...
        private void PrintButton_Click(object sender, EventArgs e)
        {
            ...
            {
                data.AttendanceDate = DateOnly.ParseExact(data.AttendanceDate, "yyyy-MM-dd", null).ToString("MMMM dd, yyyy");
                data.EmployeeName = $"{_employee.FirstName} {(string.IsNullOrEmpty(_employee.MiddleName) ? "" : $"{_employee.MiddleName[0]}. ")}{_employee.LastName}";
                data.DateIssued = $"{DateTime.Now.ToString("MM/dd/yyyy")}";
