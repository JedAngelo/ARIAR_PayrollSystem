...
namespace PayrollSystem.Forms.Modals
{
    public partial class AttendanceDateModal : Form
    {
        ...
        private async Task GetDayType(string date)
        {
            try
            {
                var apiGet = await HttpHelper.GetAsync<ApiResponse<string>>($"{ApiEndpoint.Attendance.GetAttendanceDayType}?date={date}");
                ///api/Attendance/GetAttendanceDayType?date=2025-01-16
                if (apiGet == null) throw new HttpRequestException("Can't retrieve day type, try again later");

                if (apiGet.isSuccess)
                {
                    DayTypeLabel.Text = apiGet.Data;
                }
                else
                {
                    ToastNotify.Warning("Error retrieving day type");
                }
        ...
        private void ModifyButton_Click(object sender, EventArgs e)
        {
            ...
            {
                UpdateAttendanceModal updateLogModal = new UpdateAttendanceModal(_mainForm, SelectedAttendanceLog, _date);
        ...
        {
            AddAttendanceModal addLogModal = new AddAttendanceModal(_mainForm.EmployeeInfo.Where(x => x.IsActive).ToList(), _attendanceLogs, _date);
        ...
        {
            var daySelectorModal = new DaySelectorModal(_date);
            ControlsHelper.ShowModal(_mainForm, daySelectorModal);
        }
